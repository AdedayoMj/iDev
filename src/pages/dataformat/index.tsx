...
const DataFormatPage: React.FunctionComponent = () => {
                ...
                <Header title="DATA FORMAT">
                    ...
                    <Grid container spacing={2}>
                        {DataFormat.json.map((item, index) => {
                            return <CourseCard likes={item.likes} level={item.level} key={index} title={item.name} img="/images/json.png" loading={loading} />;
                        })}
                    </Grid>
                    <div className={classes.title}>
                    ...
                    <Grid container spacing={2}>
                        {DataFormat.yaml.map((item, index) => {
                            return <CourseCard likes={item.likes} level={item.level} key={index} title={item.name} img="/images/yaml.jpg" loading={loading} />;
                        })}
                    </Grid>
                    <div className={classes.title}>
                    ...
                    <Grid container spacing={2}>
                        {DataFormat.xml.map((item, index) => {
                            return <CourseCard likes={item.likes} level={item.level} key={index} title={item.name} img="/images/xml.png" loading={loading} />;
                        })}
                    </Grid>
                    <div className={classes.title}>
                    ...
                    <Grid container spacing={2}>
                        {DataFormat.csv.map((item, index) => {
                            return <CourseCard likes={item.likes} level={item.level} key={index} title={item.name} img="/images/csv.png" loading={loading} />;
                        })}
                    </Grid>
                </Header>
            </Container>
