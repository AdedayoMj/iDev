...
const DatabasePage: React.FunctionComponent = () => {
                ...
                <Header title="DATABASE">
                    ...
                    <Grid container spacing={2}>
                        {Database.mysql.map((item, index) => {
                            return <CourseCard likes={item.likes} level={item.level} key={index} title={item.name} img="/images/mysql.jpg" loading={loading} />;
                        })}
                    </Grid>
                    <div className={classes.title}>
                    ...
                    <Grid container spacing={2}>
                        {Database.mongodb.map((item, index) => {
                            return <CourseCard likes={item.likes} level={item.level} key={index} title={item.name} img="/images/mongo.jpg" loading={loading} />;
                        })}
                    </Grid>
                    <div className={classes.title}>
                    ...
                    <Grid container spacing={2}>
                        {Database.firestore.map((item, index) => {
                            return <CourseCard likes={item.likes} level={item.level} key={index} title={item.name} img="/images/firestore.png" loading={loading} />;
                        })}
                    </Grid>
                </Header>
            </Container>
